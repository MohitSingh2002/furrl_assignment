// class used to access strings of the application.
class StringsManager {

  // OnBoarding Screen.
  static const String welcomeOnboard = 'Welcome Onboard!';
  static const String welcomeMessage = 'Hello, welcome in Furrl';
  static const String enterYourName = 'Enter your name';
  static const String enterYourEmail = 'Enter your email';
  static const String enterYourPassword = 'Enter your password';
  static const String confirmYourPassword = 'Confirm your password';
  static const String signUp = 'Sign Up';
  static const String alreadyHaveAccount = 'Already have account ? ';
  static const String signIn = 'Sign In!';

  // GenerateId Screen.
  static const String generateId = 'Generate ID';
  static const String clickMe = 'Click Me';
  static const String task3 = 'Task-3';

}
